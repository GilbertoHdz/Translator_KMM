package com.gilbertohdz.translator_kmm.android.voice_to_text.data
...
class AndroidVoiceToTextParser(
  ...
  override fun reset() {
    // _state.update { VoiceToTextParserState() } <- Could create race condition
  ...
  override fun onRmsChanged(rmsDb: Float) {
    _state.update { it.copy(
      powerRatio = rmsDb * (1f / (12f - (-2f))) // Normalizing interval of decibels
    ) }
  }
